...
> (MulanPSL-2.0). **Local difference:** a `grasp_mode: flat` path
> (flat-palm pose generation) in addition to upstream's vertical one — see the
> header of `package_manifest.yaml` for the exact delta. Everything below
> describes the shared, upstream behaviour unless it says `flat`.
